...
class Database {
   ...
   * @param {Object} config Database options (Database name)
   *
   */
  ...
  /**
   * @param {string} key
   * @returns Shortened version of Object.values() function to use objects more easily for features such as forEach and map
   */
  getValues(key) {
    if (!key) throw new DatabaseError("You did not enter a data name");
    return Object.values(file[key]);
  }
  /**
   *
   * @param {*} key
   * @returns Returns keys of datas. Shortened version of Object.keys()
   */
  getKeys(key) {
    if (!key) return Object.keys(file);
    return Object.keys(file[key]);
  }
  /**
